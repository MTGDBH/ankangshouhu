(function () {
  'use strict';
  const P=window.Pro, $=id=>document.getElementById(id);
  ...
  function switchView(view){currentView=view;views.forEach(id=>{ $('view-'+id)?.classList.toggle('hidden',id!==view);$('nav-'+id)?.classList.toggle('tab-active',id===view); });}
  ...
  function render(){if(!selected){$('currentElderlyTitle').textContent='尚未绑定长辈';$('currentStatusBadge').textContent='等待授权';$('deviceBatteryStatus').textContent='暂无设备状态';views.forEach(id=>$('view-'+id).innerHTML=P.notice('等待长辈授权','请在长辈端生成家属授权码，再点击左侧“绑定新长辈”。'));switchView(currentView);return;}
